...
<div class="max-w-[1600px] mx-auto space-y-8">
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <form method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-6 items-end">
            ...
            <div class="space-y-2">
                <button type="submit" class="w-full rounded-xl bg-blue-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-500/25 transition-all hover:bg-blue-700 hover:shadow-blue-500/40 focus:ring-2 focus:ring-blue-500/50">
                    Filtrer
                </button>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-slate-50 border-b border-slate-200">
                    ...
                    <th class="px-6 py-4 text-center text-xs font-black uppercase tracking-widest text-slate-500">Total GE</th>
                    <th class="px-6 py-4 text-center text-xs font-black uppercase tracking-widest text-slate-500">Répartition GE</th>
                    <th class="px-6 py-4 text-center text-xs font-black uppercase tracking-widest text-slate-500">Soubique</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @forelse ($statsByDren as $drenGroup)
                        @foreach ($drenGroup['ciscos'] as $stat)
                        ...
                        <tr class="bg-blue-50/70">
                            <td colspan="2" class="px-6 py-3 text-sm font-black uppercase tracking-wider text-blue-900">
                                Total DREN {{ $drenGroup['dren'] }}
                            </td>
                            <td class="px-6 py-3 text-center">
                                <span class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-blue-600 text-white text-xs font-black">
                                    {{ $drenGroup['totals']['salle'] }}
                                </span>
                            </td>
                            <td class="px-6 py-3 text-center">
                                <span class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-amber-500 text-white text-xs font-black">
                                    {{ $drenGroup['totals']['ge_total'] }}
                                </span>
                            </td>
                            <td class="px-6 py-3 text-center"></td>
                            <td class="px-6 py-3 text-center">
                                <span class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-slate-500 text-white text-xs font-black">
                                    {{ $drenGroup['totals']['soubique'] }}
                                </span>
                            </td>
                        </tr>
                    @empty
                ...
                <tfoot class="bg-slate-100 border-t-2 border-slate-300">
                    <tr>
                        ...
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-amber-500 text-white text-sm font-black">
                                {{ $totals['ge_total'] }}
                            </span>
                        </td>
