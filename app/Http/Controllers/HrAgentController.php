...
class HrAgentController extends Controller
{
    ...
    public function update(
        ...
        if (
            $previousUser
            && strtolower((string) $previousUser->email)
                !== strtolower((string) $agentModel->email)
        ) {
            $previousUser->update(['hr_agent_id' => null]);
        }

        $this->syncUserLink($agentModel);
    ...
    public function storeOwnEvent(Request $request): RedirectResponse
    {
        $agent = $request->user()?->hrAgent;

        abort_unless($agent, 403, 'Votre compte n’est pas associé à un dossier du personnel.');

        return $this->storeEvent($request, $agent->id, true);
    }

    public function storeEvent(
        Request $request,
        int $agent,
        bool $ownRequest = false
    ): RedirectResponse {
        $agentModel = HrAgent::query()->findOrFail($agent);

        abort_unless(
            $request->user()?->isAdmin()
                || ($request->user()?->hr_agent_id === $agentModel->id && $ownRequest),
            403
        );
        ...
        $data = $request->validate([
            ...
            'vehicle_type' => [
                'nullable',
                Rule::in(['administrative', 'taxi_brousse']),
            ],
        ...
        if ($data['type'] !== 'mission' && empty($data['date_debut'])) {
            return back()->withErrors(['date_debut' => 'La date de début est obligatoire pour cette demande.'])->withInput();
        }
    ...
    public function updateEventStatus(
        ...
        $data = $request->validate([
            'status' => [
                'required',
                Rule::in(['valide', 'refuse', 'annule', 'termine']),
            ],
        ]);

        $eventModel->update(['status' => $data['status']]);
   ...
   private function agentData(Request $request, ?HrAgent $agent = null): array
{
    ...
    private function syncUserLink(HrAgent $agent): void
    {
        ...
        $user = User::query()
            ...
            ->where(function ($query) use ($agent) {
                $query
                    ->whereNull('hr_agent_id')
                    ->orWhere('hr_agent_id', $agent->id);
            })
            ->first();

        $user?->update(['hr_agent_id' => $agent->id]);
    }
}