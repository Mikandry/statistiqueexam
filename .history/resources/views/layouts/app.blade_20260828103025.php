...
<head>
    ...
    <title>{{ config('app.name', 'Service de l\'organisation des examens') }} · @yield('title', 'Service de l\'organisation des Examens')</title>
...
<body class="antialiased bg-slate-50 text-slate-900">
    @php
        $hideSidebar = request()->routeIs('home', 'login', 'register');
    @endphp

    <!-- Sidebar Overlay (Only rendered inside an Espace) -->
    @if(!$hideSidebar)
        <div class="app-sidebar-overlay lg:hidden" id="appSidebarOverlay" data-sidebar-state="collapsed"></div>
    @endif

    <div class="flex min-h-screen">
        <!-- Sidebar (Only loaded outside the hub/home page) -->
        @if(!$hideSidebar)
