...
<head>
    ...
    <style>
        ...
        body { font-family: var(--app-font-sans, system-ui, -apple-system, sans-serif); }
        .ref-panel {
        ...
        .ref-collapsible-toggle { transition: transform .22s ease; }
        .ref-collapsible[open] .ref-collapsible-toggle { transform: rotate(180deg); }
        .ref-collapsible-body {
            display: grid;
            grid-template-rows: 0fr;
            transition: grid-template-rows .24s ease;
        }
        .ref-collapsible[open] .ref-collapsible-body { grid-template-rows: 1fr; }
        .ref-collapsible-inner { overflow: hidden; }
    </style>
</head>
        ...
        <main class="min-w-0 flex-1">
            <div class="ref-panel overflow-hidden rounded-[30px] backdrop-blur-sm transition-all duration-200 hover:shadow-xl">
                ...
                <div class="p-6 md:p-8">
                            ...
                            <div class="ref-collapsible-inner px-5 pb-5 md:px-6 md:pb-6">
                                <div class="grid gap-5 md:grid-cols-2">
                                    <div class="rounded-[22px] border border-slate-200 bg-slate-50/80 p-4">
                                        ...
                                        <form method="POST" action="{{ route('admin.references.dispatching-axes.store') }}" class="flex flex-col gap-3 sm:flex-row">
                                            ...
                                            <button class="ref-btn-primary px-4 py-3 text-sm shrink-0" type="submit">Ajouter axe</button>
                                        </form>
                                    </div>
                                    <div class="rounded-[22px] border border-slate-200 bg-slate-50/80 p-4">
                                        ...
                                        <form method="POST" action="{{ route('admin.references.dispatching-drop-points.store') }}" class="flex flex-col gap-3 sm:flex-row">
                                            ...
                                            <button class="ref-btn-primary px-4 py-3 text-sm shrink-0" type="submit">Ajouter point</button>
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div class="mt-5 grid gap-5 md:grid-cols-2">
                                    <div class="ref-table-wrap overflow-x-auto">
                                        <h3 class="p-4 pb-2 text-sm font-semibold uppercase tracking-wide text-slate-500">Axes configurés</h3>
                                        <table class="ref-table min-w-full border-collapse text-sm">
                                            ...
                                            @forelse($dispatchingAxes as $index => $axis)
                                                <tr>
                                                    <td>
                                                        <form id="axis-form-{{ $index }}" method="POST" action="{{ route('admin.references.dispatching-axes.update', $index) }}">
                                                            @csrf
                                                            @method('PUT')
                                                    ...
                                                    <td>
                                                        <div class="flex flex-wrap items-center gap-2">
                                                            <button class="ref-btn-primary px-3 py-2 text-sm" type="submit" form="axis-form-{{ $index }}">Modifier</button>
                                    ...
                                    <div class="ref-table-wrap overflow-x-auto">
                                        <h3 class="p-4 pb-2 text-sm font-semibold uppercase tracking-wide text-slate-500">Points de largage configurés</h3>
                                        <table class="ref-table min-w-full border-collapse text-sm">
                                            ...
                                            @forelse($dispatchingDropPoints as $index => $point)
                                                <tr>
                                                    <td>
                                                        <form id="point-form-{{ $index }}" method="POST" action="{{ route('admin.references.dispatching-drop-points.update', $index) }}">
                                                            @csrf
                                                            @method('PUT')
                                                    ...
                                                    <td>
                                                        <div class="flex flex-wrap items-center gap-2">
                                                            <button class="ref-btn-primary px-3 py-2 text-sm" type="submit" form="point-form-{{ $index }}">Modifier</button>
                    ...
                    <!-- Filter and Maintenance Management -->
                                    ...
                                    <div class="ref-table-wrap">
                                        <table class="ref-table min-w-full border-collapse text-sm">
                                            ...
                                            @forelse($drensPage as $dren)
                                                <tr>
                                                    <td>
                                                        <form id="dren-form-{{ $dren->id }}" method="POST" action="{{ route('admin.references.drens.update', $dren->id) }}">
                                                            @csrf
                                                            @method('PUT')
                                                    ...
                                                    <td>
                                                        <div class="flex flex-wrap items-center gap-2">
                                                            <button class="ref-btn-primary px-3 py-2 text-sm" type="submit" form="dren-form-{{ $dren->id }}">Modifier</button>
                                                            <form method="POST" action="{{ route('admin.references.drens.destroy', $dren->id) }}" onsubmit="return confirm('Supprimer cette DREN ?');">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button class="ref-btn-danger px-3 py-2 text-sm" type="submit">Supprimer</button>
                                                            </form>
                                            ...
                                            @empty
                                                <tr>
                                                    <td class="text-slate-500" colspan="2">Aucune DREN trouvée.</td>
                                                </tr>
                                            @endforelse
                                            </tbody>
                                        </table>
