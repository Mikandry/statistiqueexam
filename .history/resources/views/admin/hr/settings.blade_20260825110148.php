...
<div class="space-y-6 rounded-2xl bg-gradient-to-br from-slate-50 via-white to-teal-50/50 p-1">
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-md transition-all hover:shadow-lg">
        ...
        <form method="POST" action="{{ route('admin.hr.settings.update') }}" class="grid gap-4 p-6 md:grid-cols-2">
            ...
            <div class="md:col-span-2 mt-4 border-t border-slate-100 pt-5">
                ...
                <p class="mt-0.5 text-xs text-slate-400">Référence : préfixe facultatif, numéro et année.</p>
            </div>
            ...
            <label class="text-sm font-semibold text-slate-700">
                Année de référence
                <input type="number" min="2000" max="2200" name="reference_year" value="{{ old('reference_year', $settings?->reference_year ?: now()->year) }}" class="mt-1 w-full rounded-xl border-slate-300 bg-slate-50/50 px-4 py-2 text-sm shadow-sm transition-all focus:border-teal-400 focus:ring-2 focus:ring-teal-200 focus:outline-none">
            </label>
            <label class="text-sm font-semibold text-slate-700">
                Préfixe (facultatif)
                <input name="reference_prefix" value="{{ old('reference_prefix', $settings?->reference_prefix) }}" class="mt-1 w-full rounded-xl border-slate-300 bg-slate-50/50 px-4 py-2 text-sm shadow-sm transition-all focus:border-teal-400 focus:ring-2 focus:ring-teal-200 focus:outline-none">
            </label>
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-md transition-all hover:shadow-lg">
        ...
        <form method="POST" action="{{ route('admin.hr.settings.fields.update') }}" class="p-6">
            @csrf @method('PUT')
                ...
                @foreach($documents as $slug => [$icon, $label])
                    ...
                    <article class="rounded-xl border border-slate-200/70 bg-slate-50/50 p-5 transition-colors hover:border-teal-200 hover:bg-teal-50/20">
                        ...
                        <div class="grid gap-2 sm:grid-cols-2">
                            ...
                            <input type="hidden" name="fields_config[{{ $slug }}][]" value="">
