...
<div class="space-y-6 rounded-2xl bg-gradient-to-br from-slate-50 via-white to-teal-50/50 p-1">
    ...
    @if(session('status'))
    <section class="rounded-xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        ...
        <form method="POST" action="{{ route('admin.hr.settings.update') }}" class="grid gap-4 md:grid-cols-2">
            @csrf @method('PUT')
            @foreach(['ministere' => 'Ministère', 'secretariat_general' => 'Secrétariat général', 'direction_generale' => 'Direction générale', 'direction' => 'Direction', 'service' => 'Service', 'signataire' => 'Nom du signataire', 'signataire_qualite' => 'Qualité du signataire (Directeur, Chef de service...)'] as $field => $label)
