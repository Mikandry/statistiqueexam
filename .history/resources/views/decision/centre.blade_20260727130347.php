...
<div class="page-wrapper">
    ...
    <main>
        ...
        @if(!empty($isNewSession ?? false))
            <div style="background:#fff8e6;border:1px solid #ffe6a7;padding:12px;border-radius:8px;margin:12px 0;color:#663c00;">
                <strong>Nouvelle session :</strong>
                Vous avez sélectionné une année qui n'existe pas encore dans les saisies. Tous les centres sont disponibles pour saisie — vous pouvez ajouter des statistiques et créer des enregistrements pour cette session.
            </div>
        @endif

        <form method="POST" action="{{ route('decision.centre.import') }}" style="display:flex;justify-content:flex-end;gap:0.75rem;align-items:center;">
            @csrf
            <input type="hidden" name="annee" value="{{ $selectedAnnee ?? '' }}">
            <input type="hidden" name="type_examen" value="{{ $typeExamen ?? '' }}">
            <input type="hidden" name="source_annee" value="{{ $previousSessionYear ?? '' }}">
            <button type="submit" class="btn-apply" 
                @if(!auth()->user()?->isAdmin() && ($hasImportedPreviousSession ?? false)) disabled @endif
                @if(!auth()->user()?->isAdmin() && ($hasImportedPreviousSession ?? false)) title="Importation déjà effectuée pour cette session" @endif
            >Importer les centres de la session précédente</button>
        </form>
        ...
        <div class="table-card">
            <div class="table-header">
                <span class="table-title">Centres actifs pour la session</span>
            </div>

            <div style="padding:1rem 1.75rem;border-bottom:1px solid var(--border);">
                <form method="POST" action="{{ route('decision.centre.centres.store') }}" style="display:flex;gap:0.75rem;align-items:center;flex-wrap:wrap;">
                    @csrf
                    <input type="hidden" name="annee" value="{{ $selectedAnnee ?? '' }}">
                    <input type="hidden" name="type_examen" value="{{ $typeExamen ?? '' }}">
                    <select name="centre_ecrit_id" class="field-select" style="max-width:420px;" required>
                        <option value="">Ajouter un centre à cette session</option>
                        @foreach($centresNonSaisis as $centre)
                            <option value="{{ $centre['id'] }}">{{ $centre['nom'] }} — {{ $centre['region'] }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="btn-apply">Ajouter</button>
                </form>
            </div>
                    ...
                    <tbody>
                        @foreach($centresSaisis as $centre)
                            <tr>
                                <td>{{ $centre['nom'] }}</td>
                                <td>{{ $centre['region'] }}</td>
                                <td>{{ $centre['statut'] ?? 'Actif' }}</td>
                                <td>
                                    <form method="POST" action="{{ route('decision.centre.centres.destroy', ['centreDecision' => $centre['id']]) }}" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="annee" value="{{ $selectedAnnee ?? '' }}">
                                        <button type="submit" class="btn-reset" style="color:#b91c1c;">Désactiver</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                        @foreach($centresNonSaisis as $centre)
                            <tr>
                                <td>{{ $centre['nom'] }}</td>
                                <td>{{ $centre['region'] }}</td>
                                <td>{{ $centre['statut'] ?? 'Disponible' }}</td>
                                <td>-</td>
                            </tr>
                        @endforeach
                    </tbody>
