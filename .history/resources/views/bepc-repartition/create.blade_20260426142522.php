            ...
            <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50">
                ...
                <div class="p-6 md:p-10">
                    ...
                    <form method="POST" action="{{ route('bepc.repartition.store') }}">
                        ...
                        @if(isset($specialCandidates) && $specialCandidates->isNotEmpty())
                            <div class="mb-10 rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-sm">
                                <div class="mb-4 flex items-center justify-between">
                                    <div>
                                        <p class="text-sm font-black uppercase tracking-widest text-slate-400">Besoins spécifiques enregistrés</p>
                                        <p class="text-xs text-slate-500">Affiche les candidats sauvegardés pour l'année et l'examen sélectionnés.</p>
                                    </div>
                                    <span class="rounded-full bg-slate-200 px-3 py-1 text-xs font-bold uppercase text-slate-700">{{ $specialCandidates->count() }} entrées</span>
                                </div>
                                    ...
                                    <table class="min-w-full text-left text-sm text-slate-700">
                                        <thead>
                                            <tr class="border-b border-slate-200 bg-slate-100 text-slate-600">
                                                ...
                                                <th class="px-3 py-2">Centre écrit</th>
                                                <th class="px-3 py-2">Salle</th>
                                                <th class="px-3 py-2">Type handicap</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($specialCandidates as $candidate)
                                                <tr class="border-b border-slate-100">
                                                    <td class="px-3 py-2 font-semibold text-slate-800">{{ $candidate->annee }}</td>
                                                    <td class="px-3 py-2">{{ $candidate->centre_ecrit_nom }}</td>
                                                    <td class="px-3 py-2">{{ $candidate->numero_salle }}</td>
                                                    <td class="px-3 py-2">{{ $candidate->type_handicap }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
