                            ...
                            @forelse($rows as $row)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td class="px-4 py-4 text-right font-black text-indigo-600 bg-indigo-50/20">
                                        ...
                                        @if(($row['ge_probleme'] ?? 0) > 0)
                                            <div class="mt-1 text-[11px] font-semibold text-indigo-400">GE problème (3PE): {{ number_format($row['ge_probleme'], 0, ',', ' ') }}@if(!empty($row['ge_probleme_distribution'])) | Répartition: {{ implode('+', $row['ge_probleme_distribution']) }}@endif</div>
                                        @endif

                                        @if(($row['ge_autres'] ?? 0) > 0)
                                            <div class="mt-1 text-[11px] font-semibold text-indigo-400">GE autres matières (6PE): {{ number_format($row['ge_autres'], 0, ',', ' ') }}@if(!empty($row['ge_autres_distribution'])) | Répartition: {{ implode('+', $row['ge_autres_distribution']) }}@endif</div>
                                        @endif
                                    </td>
