            ...
            <div class="no-print border-b border-slate-200 p-5 md:p-6">
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function () {
                        ...
                        const allCiscos = @json($ciscos);
                        const currentCisco = @json($filters['cisco'] ?? '');
                        const drenSelect = document.getElementById('dren');
                        const ciscoSelect = document.getElementById('cisco');

                        if (!drenSelect || !ciscoSelect) {
                            return;
                        }
                        ...
                        function populateCiscoOptions(drenValue) {
                            const drenKey = String(drenValue || '');
                            const values = drenKey === ''
                                ? allCiscos
                                : (ciscosByDren[drenKey] || []);

                            ciscoSelect.innerHTML = '';
                            ciscoSelect.appendChild(buildOptions(values));

                            if (currentCisco && values.includes(currentCisco)) {
                                ciscoSelect.value = currentCisco;
                            } else {
                                ciscoSelect.value = '';
                            }
                        }
                        ...
                        drenSelect.addEventListener('change', function () {
                            populateCiscoOptions(this.value);
                        });

                        populateCiscoOptions(drenSelect.value);
                    });
                </script>
