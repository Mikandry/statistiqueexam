...
<div class="portrait-section">
    ...
    <h2>Division Automatique des PE par Centre et Compteur</h2>
    ...
    @forelse($centresByDrenCisco as $ciscoGroup)
        <div class="group-section">
            ...
            @foreach($ciscoGroup['centres'] as $centre)
                @php
                    $totalCandidates += $centre['total_candidats'];
                    $totalPe += $centre['pe'];
                @endphp
                <div class="compteur-block">
                    <div class="compteur-label">
                        Compteur: {{ $centre['centre_ecrit'] }}
                    </div>
                    <div class="centre-line">
                        <strong>Centre Écrit:</strong> {{ $centre['centre_ecrit'] }} | 
                        <strong>Centre Correction:</strong> {{ $centre['centre_correction'] }}<br>
                        <strong>Type d'examen:</strong> {{ $centre['type_examen'] }} | 
                        <strong>Année:</strong> {{ $centre['annee'] }}<br>
                        <strong>Candidats:</strong> {{ number_format($centre['total_candidats'], 0, ',', ' ') }} | 
                        <strong>Salles:</strong> {{ number_format($centre['total_salles'], 0, ',', ' ') }} | 
                        <strong>PE:</strong> 
                        @if($centre['pe'] > 0)
            ...
            <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #cbd5e1;">
                <strong>Total CISCO {{ $ciscoGroup['cisco'] }}:</strong> 
                {{ number_format($totalCandidates, 0, ',', ' ') }} candidats | 
                {{ number_format($totalPe, 0, ',', ' ') }} PE
            </div>
        </div>
    @empty
        <div class="box">
    ...
    <h2>Tableau Récapitulatif Détaillé</h2>
    <table>
        <thead>
            <tr>
                <th>DREN</th>
                <th>CISCO</th>
                <th>Centre de Correction</th>
                <th>Centre d'Écrit</th>
                <th class="right">Candidats</th>
                <th class="right">Salles</th>
                <th class="right">PE</th>
                <th class="right">GE</th>
        ...
        <tbody>
            @forelse($centreRows as $centre)
                <tr>
                    <td>{{ $centre['dren'] }}</td>
                    <td>{{ $centre['cisco'] }}</td>
                    <td>{{ $centre['centre_correction'] }}</td>
                    <td>{{ $centre['centre_ecrit'] }}</td>
