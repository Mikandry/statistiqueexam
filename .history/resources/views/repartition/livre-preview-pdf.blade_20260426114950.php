                ...
                <div class="card">
                    ...
                    @if(($filters['type_examen'] ?? '') === 'BEPC')
                        <table class="summary-table">
                            <tr>
                                <td>
                                    <div class="small-title">Total par langues</div>
                                    @forelse(($centre['langue_totals'] ?? []) as $langue => $total)
                                        <div class="langue-line">
                                            Total candidats {{ $langueLabels[$langue] ?? $langue }}:
                                            <strong>{{ number_format($total, 0, ',', ' ') }}</strong>
                                            |
                                            Total salle {{ $langueLabels[$langue] ?? $langue }}:
                                            <strong>{{ number_format((int) ($centre['langue_salles'][$langue] ?? 0), 0, ',', ' ') }}</strong>
                                        </div>
                                    @empty
                                        <div class="muted">Aucune langue.</div>
                                    @endforelse
                                </td>
                                <td class="right">
                                    <div class="summary-line">Total candidats: <strong>{{ number_format($centre['total_candidats'], 0, ',', ' ') }}</strong></div>
                                    <div class="summary-line">Total salle: <strong>{{ number_format($centre['total_salles'], 0, ',', ' ') }}</strong></div>
                                    <div class="summary-line">Total GE/Matiere: <strong>{{ number_format($centre['ge_count'], 0, ',', ' ') }}</strong></div>
                                </td>
                            </tr>
                        </table>
                    @endif
