...
<div class="portrait-section">
    ...
    @php
        ...
        // Fonction pour obtenir les noms de compteurs pour un groupe
        function getCompteurNamesForGroup($groupNum, $compteurNames) {
            $names = isset($compteurNames[$groupNum]) 
                ? array_map('trim', array_filter(explode("\n", (string)$compteurNames[$groupNum])))
                : [];
            
            if (empty($names)) {
                // Générer des noms par défaut si non fournis
                $names = ['Compteur ' . $groupNum];
            }
        ...
        function distributeCompteursCenters($centres, $compteurNamesList) {
            $centresArray = $centres->toArray();
            $count = count($centresArray);
            $numCompteurs = count($compteurNamesList);
            
            if ($count === 0 || $numCompteurs === 0) {
                return [];
            }
            
            $compteurs = [];
            $centresPerCompteur = ceil($count / $numCompteurs);
            
            for ($i = 0; $i < $numCompteurs; $i++) {
                ...
                if (!empty($compteurCentres)) {
                    ...
                    foreach ($compteurCentres as $centre) {
                        ...
                        if ($pe > 0) {
                            $lastPe = $firstPe + $pe - 1;
                            $peByCenter[] = $centreLabel . ' PE ' . $firstPe . '-' . $lastPe;
                            $firstPe = $lastPe + 1;
                        } else {
                            $peByCenter[] = $centreLabel . ' (sans PE)';
                        }
                    }
                    
                    $compteurs[] = [
                        'name' => $compteurNamesList[$i],
                        'centres' => $compteurCentres,
                        'peByCenter' => $peByCenter,
    ...
    @forelse($centresByDrenCisco as $ciscoGroup)
        @php
            // Déterminer le groupe basé sur le nombre de groupes configurés
            // Pour simplifier, utiliser le groupe 1 par défaut ou faire un mapping
            $groupNum = 1;
            $compteurNamesList = getCompteurNamesForGroup($groupNum, $compteurNames);
            $compteurs = distributeCompteursCenters(collect($ciscoGroup['centres']), $compteurNamesList);
        @endphp
            ...
            @foreach($compteurs as $compteur)
                <div class="compteur-block">
                    ...
                    <div class="centre-line">
                        @foreach($compteur['peByCenter'] as $pe)
                            {{ $pe }}<br>
                        @endforeach
        ...
        foreach ($centresByDrenCisco as $ciscoGroup) {
            ...
            foreach ($compteurs as $compteur) {
                $fullRecap[] = [
                    ...
                    'peByCenter' => $compteur['peByCenter'],
        ...
        <tbody>
            @forelse($fullRecap as $recap)
                <tr>
                    <td>{{ $recap['dren'] }}</td>
                    <td>{{ $recap['cisco'] }}</td>
                    <td><strong>{{ $recap['compteur_name'] }}</strong></td>
                    <td>
                        @foreach($recap['peByCenter'] as $pe)
                            {{ $pe }}<br>
                        @endforeach
                    </td>
                    <td class="right">{{ number_format($recap['total_candidats'], 0, ',', ' ') }}</td>
                    <td class="right">{{ number_format($recap['total_pe'], 0, ',', ' ') }}</td>
                </tr>
            @empty
                <tr>
