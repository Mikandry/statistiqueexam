            ...
            <div class="no-print border-b border-slate-200 p-5 md:p-6">
                ...
                @if($filters['dren'] !== '')
                    <div class="mt-5 rounded-lg border border-purple-200 bg-purple-50 p-4">
                        <h3 class="mb-4 text-sm font-bold text-purple-900">📋 ORDRE D'IMPRESSION - Réorganiser les CISCOs</h3>
                        <div id="cisco-order-container" class="space-y-2">
                            @php
                                $drenData = null;
                                foreach ($centrePagesByDren as $data) {
                                    if ($data['dren'] === $filters['dren']) {
                                        $drenData = $data;
                                        break;
                                    }
                                }
                                $drenCiscos = [];
                                if ($drenData) {
                                    foreach ($drenData['pages'] as $page) {
                                        if (!empty($page) && isset($page[0]['cisco'])) {
                                            $cisco = $page[0]['cisco'];
                                            if (!in_array($cisco, $drenCiscos)) {
                                                $drenCiscos[] = $cisco;
                                            }
                                        }
                                    }
                            ...
                            @if(count($drenCiscos) > 0)
                                <form id="cisco-order-form" method="POST" action="{{ route('repartition.livre.cisco-order') }}" class="space-y-2">
                                    @csrf
                                    ...
                                    <div id="sortable-cisco-list" class="space-y-1">
                                        @foreach($drenCiscos as $index => $cisco)
                                            <div class="cisco-item flex items-center gap-2 rounded-lg border border-purple-300 bg-white p-3 cursor-move hover:bg-purple-50 transition" draggable="true" data-cisco="{{ $cisco }}">
                                                <span class="text-purple-600 font-bold">≡</span>
                                                <span class="flex-1 font-medium text-slate-700">{{ $index + 1 }}. {{ $cisco }}</span>
                                    ...
                                    <div class="mt-4 flex gap-2">
                                        <button type="button" onclick="resetCiscoOrder()" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                                            Réinitialiser
                                        </button>
                                ...
                                document.addEventListener('DOMContentLoaded', function() {
                                    const sortableList = document.getElementById('sortable-cisco-list');
                                    let draggedElement = null;

                                    function updateHiddenInputs() {
                                        const items = sortableList.querySelectorAll('.cisco-item');
                                        const form = document.getElementById('cisco-order-form');
                                        
                                        // Effacer tous les inputs cisco_order[]
                                        form.querySelectorAll('input[name="cisco_order[]"]').forEach(input => input.remove());
                                        
                                        // Ajouter les inputs dans le nouvel ordre
                                        items.forEach(item => {
                                            const ciscoValue = item.getAttribute('data-cisco');
                                            const input = document.createElement('input');
                                            input.type = 'hidden';
                                            input.name = 'cisco_order[]';
                                            input.value = ciscoValue;
                                            form.appendChild(input);
                                        });
                                    }

                                    sortableList.addEventListener('dragstart', function(e) {
                                        if (e.target.classList.contains('cisco-item')) {
                                    ...
                                    function getDragAfterElement(container, y) {
                                        const draggableElements = [...container.querySelectorAll('.cisco-item:not([style*="opacity"])')];
                                ...
                                function resetCiscoOrder() {
                                    if (confirm('Êtes-vous sûr de vouloir réinitialiser l\'ordre ?')) {
                                        location.reload();
                                    }
                                }
                                </script>
                            @else
                                <div class="rounded-lg bg-white px-3 py-2 text-sm text-slate-500">
                                    Aucune CISCO disponible pour cette DREN.
                                </div>
                            @endif
                        </div>
                    </div>
