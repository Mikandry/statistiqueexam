...
<div class="fixed-footer">
    ...
    <span> | produced by: RAMAROSON Andry Michael V1.0</span>
</div>
@php
    ...
    $defaultCompteurNames = ['Compteur 1', 'Compteur 2'];
    $buildCompteurRanges = function (int $totalPe, array $names) {
        if ($totalPe <= 0 || count($names) === 0) {
            return array_fill(0, count($names), '-');
        }

        $compteurCount = count($names);
        $base = intdiv($totalPe, $compteurCount);
        $extra = $totalPe % $compteurCount;
        $start = 1;
        $ranges = [];

        for ($index = 0; $index < $compteurCount; $index++) {
            ...
            $end = $start + $count - 1;
            $ranges[] = $start === $end
                ? sprintf('%s: PE%s', $names[$index], $start)
                : sprintf('%s: PE%s-PE%s', $names[$index], $start, $end);
            $start = $end + 1;
        }
    ...
    <div class="page {{ $hasPagesAfterRecap ? 'forced-break' : '' }}">
        ...
        <div class="card">
            ...
            <div class="card" style="margin-top:8px;">
                <div class="small-title">Noms des compteurs</div>
                <table class="pdf-table">
                    <thead>
                        <tr>
                            <th>Centre écrit</th>
                            @foreach($defaultCompteurNames as $name)
                                <th>{{ $name }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($recap['rows'] as $row)
                            @php $counterRanges = $buildCompteurRanges((int) ($row['pe'] ?? 0), $defaultCompteurNames); @endphp
                            <tr>
                                <td>{{ $row['centre_ecrit'] }}</td>
                                @foreach($counterRanges as $range)
                                    <td>{{ $range }}</td>
                                @endforeach
                            </tr>
                        @endforeach
