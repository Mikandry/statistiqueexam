...
<script>
    (function () {
        ...
        function renderGroups() {
            ...
            groupsContainer.innerHTML = groups.map(group => `
                <div class="rounded-lg border border-indigo-200 bg-indigo-50/50 p-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-medium text-indigo-700 mb-1">Régions assignées automatiquement:</label>
                            <div class="w-full rounded border border-indigo-300 bg-gray-50 px-2 py-2 text-xs min-h-[60px]">${group.regions.join('<br>')}</div>
                        </div>
                        <div>
            ...
            // Add event listeners for names only
        ...
        function renderMatrix() {
            ...
            matrixBody.innerHTML = keys.map((key) => {
                ...
                const groupForRegion = groupData.find(group => 
                    group.regions.some(region => key.includes(region) || region.includes(key))
                );
                const groupInfo = groupForRegion ? `Groupe ${groupForRegion.id}` : '-';
