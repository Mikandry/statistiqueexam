...
<head>
    ...
    <style>
        ...
        .metric-card { transition: all 0.3s ease; border: 1px solid rgba(226, 232, 240, 0.8); }
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                    @php 
                        $cards = [
                            ...
                            ['Centres', $globalStats['total_centres_ecrit'], 'fa-map-marker-alt', 'emerald']
                        ];
                    @endphp
                    @foreach($cards as $card)
                    <div class="metric-card group rounded-3xl bg-white p-6 shadow-sm">
                        <div class="rounded-2xl bg-{{ $card[2] ?? 'slate' }}-50 p-3 w-fit text-{{ $card[3] }}-600 mb-4 group-hover:bg-{{ $card[3] }}-600 group-hover:text-white transition-colors">
                            <i class="fas {{ $card[2] }} text-xl"></i>
                        </div>
                        <p class="text-[10px] font-black text-slate-400 uppercase">{{ $card[0] }}</p>
                        <h3 class="text-3xl font-black text-slate-900 mt-1">{{ number_format($card[1], 0, ',', ' ') }}</h3>
                    </div>
                    @endforeach

                    <div class="rounded-3xl border border-slate-900 bg-slate-900 p-6 shadow-xl text-white">
                        @php
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            drenSelect.addEventListener('change', () => {
                const selected = drenSelect.value;
                ciscoSelect.innerHTML = '<option value="">Tous</option>';
                if(selected && ciscosByDren[selected]) {
                    ciscosByDren[selected].forEach(c => {
