...
@endif
@if($title === 'Fiche de congé')<p class="text">La présente fiche constate l’autorisation de congé accordée à l’agent désigné ci-dessus.</p>@elseif($title === 'Attestation de non-jouissance de congé')<p class="text">Après vérification des registres disponibles, il est attesté que l’agent désigné ci-dessus n’a pas bénéficié de congé durant {{ $period }}. La présente attestation est délivrée pour servir et valoir ce que de droit.</p>@elseif($title === 'Autorisation d’absence')<p class="text">L’autorisation d’absence est accordée pour la période indiquée.</p>@elseif($title === 'Ordre de mission')<p class="text">L’agent est chargé d’effectuer la mission décrite ci-dessus pendant la période indiquée.</p>@else<p class="text">La présente autorisation permet à l’agent de participer à l’activité de formation indiquée.</p>@endif
