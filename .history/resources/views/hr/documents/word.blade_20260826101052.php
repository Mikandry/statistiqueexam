<div style="text-align:left">
    <img src="{{ public_path('images/drapeau-madagascar.jpeg') }}" width="75" />
</div>
...
@if($event)
    <p><b>Objet :</b> {{ $event->title ?: \App\Models\HrEvent::TYPES[$event->type] }}<br /><b>Période :</b> du {{ $event->date_debut?->format('d/m/Y') }} @if($event->date_fin) au {{ $event->date_fin->format('d/m/Y') }} @endif<br /><b>Nombre de jours :</b> {{ ($event->date_fin ?: $event->date_debut)->diffInDays($event->date_debut) + 1 }}</p>
    @if($title === 'ORDRE DE MISSION')
        @if($event->destination)<p><b>Destination :</b> {{ $event->destination }}</p>@endif
        <p><b>Véhicule :</b> {{ $event->vehicle_type === 'administrative' ? 'Voiture administrative' : ($event->vehicle_type === 'taxi_brousse' ? 'Taxi-brousse' : 'Non précisé') }}</p>
    @endif
@endif
...
@elseif($title === 'ORDRE DE MISSION')
    @if($event)
        <p><b>Véhicule :</b> {{ $event->vehicle_type === 'administrative' ? 'Voiture administrative' : ($event->vehicle_type === 'taxi_brousse' ? 'Taxi-brousse' : 'Non précisé') }}</p>
    @endif
    <p>Le présent ordre de mission lui est délivré pour servir et valoir ce que de droit et lui tient lieu d’ordre de route.</p>
@else
