...
<p style="text-align:right">{{ $settings?->reference_prefix ?: 'N°' }} {{ $settings?->next_reference_number ?: 1 }}/{{ $settings?->reference_year ?: $today->year }}</p>
...
@if($title === 'Attestation de non-jouissance de congé')<p>Après vérification des registres disponibles, l’intéressé(e) n’a pas bénéficié de congé pendant {{ $period ?? 'l’année en cours' }}. La présente attestation est délivrée pour servir et valoir ce que de droit.</p>@else<p>La présente attestation est délivrée pour servir et valoir ce que de droit.</p>@endif
