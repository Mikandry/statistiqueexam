...
<div class="space-y-6">
    ...
    <section class="grid gap-6 xl:grid-cols-2">
        ...
        <div class="rounded-lg border bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="{{ route('hr.agents.events.store', $agent->id) }}" class="grid gap-4 md:grid-cols-2">
                @csrf
                <select name="type" class="rounded-lg border-slate-300 text-sm">@foreach(\App\Models\HrEvent::TYPES as $value => $label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select>
