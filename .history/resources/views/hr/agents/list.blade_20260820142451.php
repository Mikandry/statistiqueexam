...
<div class="space-y-6">
    ...
    <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        @if(auth()->user()?->isAdmin())<div class="mb-4 flex justify-end"><a href="{{ route('admin.hr.settings') }}" class="rounded-lg border border-slate-300 px-3 py-2 text-sm font-bold">Paramètres des documents RH</a></div>@endif
                ...
                <article class="rounded-lg border border-slate-200 p-4">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @php($validEvents = $agent->events->filter(fn ($item) => ! in_array($item->status, ['refuse', 'annule'], true)))
                            @php($totalDays = $validEvents->sum(fn ($item) => ($item->date_fin ?: $item->date_debut)->diffInDays($item->date_debut) + 1))
                            @php($absenceDays = $validEvents->where('type', 'autorisation_absence')->sum(fn ($item) => ($item->date_fin ?: $item->date_debut)->diffInDays($item->date_debut) + 1))
                            @php($leaveDays = $validEvents->where('type', 'conge')->sum(fn ($item) => ($item->date_fin ?: $item->date_debut)->diffInDays($item->date_debut) + 1))
                            @php($requestedCount = $validEvents->filter(fn ($item) => in_array($item->status, ['demande', 'valide', 'termine'], true))->count())
                            <a href="{{ route('hr.documents.non-leave', $agent->id) }}" class="rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white">PDF non-jouissance</a>
                            <a href="{{ route('hr.documents.preview', [$agent->id, 'non-jouissance']) }}" target="_blank" class="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold">Aperçu</a>
                            <a href="{{ route('hr.documents.word', [$agent->id, 'non-jouissance']) }}" class="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white">Word</a>
                            <span class="rounded-lg bg-blue-50 px-3 py-2 text-xs font-bold text-blue-700">{{ $agent->events->count() }} situation(s)</span>
                        </div>
                    </div>

                    <div class="mt-4 grid gap-2 sm:grid-cols-3">
                        <div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs font-bold uppercase text-slate-500">Jours absence</p><p class="text-xl font-black">{{ $absenceDays }}</p></div>
                        <div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs font-bold uppercase text-slate-500">Jours congé</p><p class="text-xl font-black">{{ $leaveDays }}</p></div>
                        <div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs font-bold uppercase text-slate-500">Total indisponibilité</p><p class="text-xl font-black">{{ $totalDays }}</p></div>
                        <div class="rounded border border-slate-200 bg-slate-50 p-3"><p class="text-xs font-bold uppercase text-slate-500">Demandes suivies</p><p class="text-xl font-black">{{ $requestedCount }}</p></div>
                    </div>
                        ...
                        <div class="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
                            ...
                            <div class="mt-2 flex flex-wrap gap-2">
                                @foreach($agent->events as $event)
                                    ...
                                    @endif
                                    @if(isset($documentType))<a href="{{ route('hr.documents.preview', [$agent->id, $documentType, $event->id]) }}" target="_blank" class="rounded border border-slate-300 px-2 py-1 text-xs font-bold">Aperçu</a><a href="{{ route('hr.documents.word', [$agent->id, $documentType, $event->id]) }}" class="rounded bg-blue-600 px-2 py-1 text-xs font-bold text-white">Word</a>@endif
                                    <span class="rounded bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700">{{ ($event->date_fin ?: $event->date_debut)->diffInDays($event->date_debut) + 1 }} jour(s)</span>
