...
<div class="space-y-6 rounded-2xl bg-gradient-to-br from-slate-50 via-white to-emerald-50/40 p-1">
    ...
    <section class="grid gap-6 xl:grid-cols-2">
        ...
        <div class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
            <h2 class="mb-4 font-black">Déposer une demande RH</h2>
            <form method="POST" action="{{ ($isAdmin ?? false) ? route('hr.agents.events.store', $agent->id) : route('hr.my-events.store') }}" class="grid gap-4 md:grid-cols-2">
                ...
                <select name="type" class="rounded-lg border-slate-300 text-sm">
                    @foreach(\App\Models\HrEvent::TYPES as $value => $label)
                        @if(($isAdmin ?? false) || in_array($value, ['conge', 'autorisation_absence', 'formation', 'mission', 'autre'], true))
                ...
                @if($isAdmin ?? false)
                    <select name="status" class="rounded-lg border-slate-300 text-sm">
                        @foreach(\App\Models\HrEvent::STATUSES as $value => $label)
                            <option value="{{ $value }}" @selected($value === 'valide')>{{ $label }}</option>
                        @endforeach
                    </select>
                @else
                ...
                <label class="text-sm font-semibold text-slate-700">
                    Voiture
                    <select name="vehicle_type" class="mt-1 w-full rounded-lg border-slate-300 text-sm">
                        <option value="">Non précisée</option>
                        <option value="administrative">Voiture administrative</option>
    ...
    @php
        $daysFor = fn ($event) => $event->date_debut
            ? ($event->date_fin ?: $event->date_debut)->diffInDays($event->date_debut) + 1
            : 0;
            ...
            <table class="min-w-full text-sm">
                ...
                @forelse($agent->events as $event)
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            {{ $event->date_debut?->format('d/m/Y') ?: 'Date non précisée' }}
