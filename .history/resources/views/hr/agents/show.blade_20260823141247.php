...
<div class="space-y-6 rounded-2xl bg-gradient-to-br from-slate-50 via-white to-emerald-50/40 p-1">
    ...
    <section class="grid gap-6 xl:grid-cols-2">
        ...
        <div class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
            ...
            <form method="POST" action="\{\{ ($isAdmin ?? false) ? route('hr.agents.events.store', $agent->id) : route('hr.my-events.store') \}\}" class="grid gap-4 md:grid-cols-2">
                ...
                <select name="type" class="rounded-lg border-slate-300 text-sm">
                    @foreach(\App\Models\HrEvent::TYPES as $value => $label)
                        @if(($isAdmin ?? false) || in_array($value, ['conge', 'autorisation_absence'], true))
                            <option value="{{ $préférence }}">{{ $label }}</option>
                        @endif
                    @endforeach
                </select>
                @if($isAdmin ?? false)
                    <select name="status" class="rounded-lg border-slate-300 text-sm">
                        @foreach(\App\Models\HrEvent::STATUSES as $value => $label)
                            <option value="{{ $value }}" @selected($value === 'demande')>{{ $label }}</option>
                        @endforeach
                    </select>
                @else
                ...
                <button class="rounded-lg bg-blue-600 px-5 py-2 text-sm font-bold text-white md:col-span-2">Enrgistrer la demande</button>
            </form>
        </div>
    </section>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm"><p class="text-xs font-bold uppercase text-slate-500">Jours de congé</p><p class="text-2xl font-black">{{ $leaveDays }}</p></div>
        <div class="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm"><p class="text-xs font-bold uppercase text-slate-500">Jours d'autorisation</p><p class="text-2xl font-black">{{ $absenceDays }}</p></div>
        <div class="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm"><p class="text-xs font-bold uppercase text-slate-500">Total indisponibilité</p><p class="text-2xl font-black">{{ $totalDays }}</p></div>
        <div class="rounded-xl border border-slate-200/70 bg-white p-4 shadow-sm"><p class="text-xs font-bold uppercase text-slate-500">Demandes en attente</p><p class="text-2xl font-black">{{ $requestedCount }}</p></div>
    </section>

    {{-- IDENTITE --}}
    <section class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">

        <h2 class="mb-4 font-black">Informations administratives</h2>

        <div class="grid gap-4 md:grid-cols-3">

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Nom et prénoms</p>
                <p class="font-semibold">{{ $agent->full_name }}</p>
            </div>

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Matricule</p>
                <p>{{ $agent->matricule ?: '—' }}</p>
            </div>
            ...
            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Corps</p>
                <p>{{ $agent->corps ?: '—' }}</p>
            </div>

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Grade</p>
                <p>{{ $agent->grade ?: '—' }}</p>
            </div>

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Indice</p>
                <p>{{ $agent->indice ?: '—' }}</p>
            </div>

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Catégorie</p>
                <p>{{ $agent->categorie ?: '—' }}</p>
            </div>

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Échelon</p>
                <p>{{ $agent->echelon ?: '—' }}</p>
            </div>

            <div>
                <p class="text-xs font-bold uppercase text-slate-500">Fonction</p>
                <p>{{ $agent->fonction ?: '—' }}</p>
            </div>
    ...
    <section class="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
        ...
        <h2 class="mb-4 font-black">Affectation actuelle</h2>

        @if($agent->currentAssignment)

            <div class="grid gap-4 md:grid-cols-4">
                <div>
                    <p class="text-xs font-bold uppercase text-slate-500">Direction</p>
                    <p>{{ $agent->currentAssignment->direction ?: '—' }}</p>
                </div>
                <div>
                    <p class="text-xs font-bold uppercase text-slate-500">Service</p>
                    <p>{{ $agent->currentAssignment->service ?: '—' }}</p>
                </div>
                <div>
                    <p class="text-xs font-bold uppercase text-slate-500">Bureau</p>
    ...
    <section class="rounded-2xl border border-slate-200/70 bg-white shadow-sm">
        ...
        <div class="border-b p-5">
            <h2 class="font-black">Historique des situations</h2>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            ...
                            @elseif($event->type === 'mission')
                                @php($documentype = 'mission')
                            @elseif($event->type === 'formation')
                                @php($documentype = 'formation')
                            @endif

                            @if($documentType && (($isAdmin ?? false) || $event->status === 'valide'))
                                <div class="flex flex-wrap gap-1">
                                    <a href="{{ route('hr.documents.preview', [$agent->id, $documentype, $event->id]) }} target="_blank" class="rounded border border-slate-300 px-2 py-1 text-xs font-bold text-slate-700">Aperçu</a>
                                    <a href="{{ route('hr.documents.word', [$agent->id, $documentype, $event->id]) }}" class="rounded bg-blue-600 px-2 py-1 text-xs font-bold text-white">Word</a>
                                    <a href="{{ route('hr.documents.pdf', [$agent->id, $documentype, $event->id]) }}" class="rounded bg-rose-600 px-2 py-1 text-xs font-bold text-white">PDF</a>
                                </div>
                            @elseif($documentType)
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200/70 bg-gradient-to-br from-white to-slate-50/80 shadow-md transition-all hover:shadow-lg">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 bg-gradient-to-r from-teal-50/70 to-emerald-50/40 p-5">
            <div>
                <h2 class="text-lg font-black text-slate-900">📄 Attestations et documents</h2>
                <p class="mt-1 text-sm text-slate-500">Générer des documents administratifs pour cet agent.</p>
            </div>
            <a
                href="{{ route('hr.documents.index', $agent->id) }}"
                class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-600 to-emerald-600 px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-teal-200/50 transition-all hover:scale-105 hover:shadow-lg active:scale-95"
            >
        ...
        <div class="grid gap-3 p-5 sm:grid-cols-3">
            ...
            @php
                $availableDocs = [
                    ['slug' => 'non-interruption', 'label' => 'Non-interruption de service', 'icon' => '🔄', 'color' => 'border-cyan-200 bg-cyan-50 text-cyan-700'],
                    ['slug' => 'non-jouissance', 'label' => 'Non-jouissance de congé', 'icon' => '📄', 'color' => 'border-slate-200 bg-slate-50 text-slate-700'],
                    ['slug' => 'prise-service', 'label' => 'Fiche de prise de service', 'icon' => '🚀', 'color' => 'border-emerald-200 bg-emerald-50 text-emerald-700'],
                    ['slug' => 'conge', 'label' => 'Fiche de congé', 'icon' => '🌴', 'color' => 'border-blue-200 bg-blue-50 text-blue-700'],
                    ['slug' => 'mission', 'label' => 'Ordre de mission', 'icon' => '📋', 'color' => 'border-violet-200 bg-violet-50 text-violet-700'],
                    ['slug' => 'formation', 'label' => 'Demande de formation', 'icon' => '📚', 'color' => 'border-pink-200 bg-pink-50 text-pink-700'],
                ];
            @endphp

            @foreach($availableDocs as $doc)
                <div class="flex items-center gap-3 rounded-xl border {{ $doc['color'] }} px-4 py-3 transition-all hover:shadow-md">
                    <span class="text-2xl">{{ $doc['icon'] }}</span>
                    <div class="min-w-0">
                        <p class="text-xs font-black uppercase tracking-wider opacity-80">Document</p>
                        <p class="truncate text-sm font-bold">{{ $doc['label'] }}</p>
                    </div>
                </div>
            @endforeach

            <div class="flex items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50/60 px-4 py-6 text-center">
                <a href="{{ route('hr.documents.index', $agent->id) }}" class="text-sm font-bold text-teal-700 hover:text-teal-900 hover:underline">
