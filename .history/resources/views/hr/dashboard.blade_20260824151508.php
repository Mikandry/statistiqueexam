                ...
                @forelse($situations as $s)
                    ...
                    <tr class="transition-colors duration-200 hover:bg-slate-50/80 group">
                        ...
                        <td class="px-4 py-4">
                            ...
                            @php
                                $situationStyle = match (true) {
                                    $s['code'] === 'present' => 'border-emerald-300 bg-emerald-50 text-emerald-700',
                                    $s['code'] === 'conge' => 'border-blue-300 bg-blue-50 text-blue-700',
                                    str_starts_with($s['code'], 'formation') => 'border-violet-300 bg-violet-50 text-violet-700',
                                    str_starts_with($s['code'], 'autorisation_absence') => 'border-orange-300 bg-orange-50 text-orange-700',
                                    $s['code'] === 'mission' => 'border-amber-300 bg-amber-50 text-amber-700',
                                    $s['code'] === 'affectation_temporaire' => 'border-cyan-300 bg-cyan-50 text-cyan-700',
                                    $s['code'] === 'sans_affectation' => 'border-slate-300 bg-slate-50 text-slate-700',
                                    default => 'border-amber-300 bg-amber-50 text-amber-700',
                                };
                                $situationIcon = match (true) {
                                    $s['code'] === 'present' => '✅',
                                    $s['code'] === 'conge' => '🌴',
                                    str_starts_with($s['code'], 'formation') => '📚',
                                    str_starts_with($s['code'], 'autorisation_absence') => '📝',
                                    $s['code'] === 'mission' => '📋',
                                    $s['code'] === 'affectation_temporaire' => '🔄',
                                    $s['code'] === 'sans_affectation' => '🚫',
                                    default => '📌',
                                };
                            @endphp

                            <span class="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-bold {{ $situationStyle }}">
                                {{ $situationIcon }}
