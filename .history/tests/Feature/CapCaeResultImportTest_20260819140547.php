...
class CapCaeResultImportTest extends TestCase
{
    public function test_it_imports_results_and_generates_independent_orders(): void
    {
        ...
        $this->actingAs($user)->get(route('cap-cae-results.preview', $batch))->assertOk()->assertSee('Date du PV');
        ...
        $batch->update(['list_status' => 'admissible']);
        $this->actingAs($user)->get(route('cap-cae-results.preview', $batch))->assertOk()->assertDontSee('Date du PV');
    }
}
