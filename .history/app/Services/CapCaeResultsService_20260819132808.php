...
class CapCaeResultsService
{
    private const REQUIRED_COLUMNS = [
        ...
        'service_location' => ['localite de service', 'localite service', 'lieu de service'],
    ...
    public function import(UploadedFile $file, string $examType, int $year, array $signature, ?int $userId): CapCaeResultBatch
    {
        ...
        foreach (array_slice($rows, $headerRow) as $offset => $row) {
            ...
            $prepared[] = [
                ...
                'service_location' => $this->text($row, $headers['service_location']),
                'dren' => $dren,
                'centre' => $centre,
    ...
    {
        return [
            'number' => 'N°', 'name' => 'Nom et prénoms', 'birth_date' => 'Date de naissance',
            'service_location' => 'Localité de service', 'dren' => 'DREN', 'centre' => 'Centre',
        ][$column] ?? $column;
    }
}
