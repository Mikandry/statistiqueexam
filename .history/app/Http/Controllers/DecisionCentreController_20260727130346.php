...
class DecisionCentreController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($previousSessionYear !== null) {
            ...
            $hasImportedPreviousSession = $previousDecisions->isNotEmpty()
                && $previousDecisions->diff($currentDecisionIds)->isEmpty();
        }
        ...
        return view('decision.centre', compact(
            ...
            'hasImportedPreviousSession', 'previousSessionYear'
        ));
    }
    ...
    public function import(Request $request)
    {
        ...
        if ($annee === '') {
            return redirect()->route('decision.centre', ['annee' => $annee, 'type_examen' => $typeExamen])
                ->withErrors(['annee' => 'Veuillez sélectionner une année de session.']);
        }
        ...
        if ($importedCount === 0) {
            return redirect()->route('decision.centre', ['annee' => $annee, 'type_examen' => $typeExamen])
                ->with('status', 'Aucune décision de centre n’a été trouvée à importer pour cette session.');
        }

        return redirect()->route('decision.centre', ['annee' => $annee, 'type_examen' => $typeExamen])
            ->with('status', "{$importedCount} centre(s) importé(s) vers la session {$annee}.");
    }
