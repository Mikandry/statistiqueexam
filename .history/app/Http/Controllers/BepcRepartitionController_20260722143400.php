...
class BepcRepartitionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $selectedAnnee = trim((string) $request->query('annee', $stickyContext['annee'] ?? ''));

        $sessionDecisions = CentreDecision::getActiveForSession($selectedAnnee, $typeExamen);
        $sessionCentreEcritIds = $sessionDecisions->pluck('centre_ecrit_id')->filter()->unique()->values();
        ...
        $centresEcritDisponibles = CentreEcrit::query()
            ...
            ->when($sessionCentreEcritIds->isNotEmpty(), function ($query) use ($sessionCentreEcritIds) {
                $query->whereIn('centre_ecrits.id', $sessionCentreEcritIds->all());
            })
