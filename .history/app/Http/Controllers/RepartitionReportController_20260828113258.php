...
class RepartitionReportController extends Controller
{
    ...
    private function buildStatsReportPayload(Request $request): array
    {
        ...
        $previousDrenTotalsFromCentres = $previousRows
            ->groupBy('dren')
            ->map(function (Collection $group, string $dren) {
                return (object) [
                    'dren' => $dren,
        ...
        $previousCiscoTotalsFromCentres = $previousRows
        ...
        $previousCiscoTotals = $previousCiscoTotals->isEmpty()
            ? $previousCiscoTotalsFromCentres
            : $previousCiscoTotals;
        ...
        $drenComparison = collect(array_unique(array_merge(
            ...
            $previousDrenTotalsFromCentres->keys()->all()
        )))->map(function (string $dren) use ($recapByDren, $previousDrenRows, $previousDrenTotalsFromCentres, $currentDrenLangues) {
            $current = $recapByDren->first(fn (array $row) => $row['dren'] === $dren);
            $previous = $previousDrenRows->get($dren) ?? $previousDrenTotalsFromCentres->get($dren);
            $currentC = (int) ($current['total_candidats'] ?? 0);
            $previousC = (int) ($previous->total_candidats ?? 0);
            $currentS = (int) ($current['total_salles'] ?? 0);
            $previousS = (int) ($previous->total_salles ?? 0);
    ...
    private function normalizeImportValue(string $value): string
    {
        ...
        if (preg_match('/[\x80-\x9F]/', $value) === 1) {
            $converted = @iconv('MACINTOSH', 'UTF-8//IGNORE', $value);

            return $converted !== false ? $converted : mb_convert_encoding($value, 'UTF-8', 'CP850');
        }

        $encoding = mb_detect_encoding($value, ['Windows-1252', 'ISO-8859-1'], true) ?: 'Windows-1252';
